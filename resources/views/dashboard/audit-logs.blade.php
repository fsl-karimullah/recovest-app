...
<div class="space-y-6">
    ...
    <div class="bg-slate-900 p-5 rounded-2xl border border-slate-800">
        <h3 class="text-base font-bold text-white">Log Otorisasi & Kepatuhan Keuangan</h3>
        <p class="text-xs text-slate-400">Pencatatan riwayat setiap perubahan data transaksi kas, faktur, dan rekonsiliasi untuk pencegahan *fraud* internal.</p>
    </div>
                ...
                <tbody class="divide-y divide-slate-800/60 text-slate-300">
                    @forelse($logs as $log)
                        <tr class="hover:bg-slate-800/40">
                            <td class="p-3.5 font-mono text-slate-400">{{ $log->created_at->format('Y-m-d H:i:s') }}</td>
                            <td class="p-3.5 font-bold text-white">{{ $log->user->name ?? 'System' }}</td>
                            <td class="p-3.5 font-semibold text-blue-400">{{ $log->module }}</td>
                            <td class="p-3.5">
                                <span class="px-2 py-0.5 rounded text-[10px] font-bold bg-slate-800 text-slate-300">
                                    {{ $log->action }}
                                </span>
                            </td>
                            <td class="p-3.5 text-slate-300">{{ $log->description }}</td>
                            <td class="p-3.5 font-mono text-slate-500">{{ $log->ip_address ?? '127.0.0.1' }}</td>
                        </tr>
                    @empty
                        <tr>
                            <td colspan="6" class="p-6 text-center text-slate-500">Belum ada catatan log aktivitas.</td>
                        </tr>
                    @endforelse
                </tbody>
            </table>
